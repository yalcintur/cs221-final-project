...
class EV2GymEncoded(gym.Env):
    ...
    def __init__(self,
        ...
        super(EV2GymEncoded, self).__init__()
    ...
    def _get_observation(self):
        ...
        """
        raw_observation = self.state_function(self)
        if self.scaler and self.encoder:
            # Scale and encode the observation
            normalized_obs = self.scaler.transform([raw_observation])  # Normalize
            tensor_obs = torch.tensor(normalized_obs, dtype=torch.float32)  # Convert to tensor
            with torch.no_grad():
                encoded_obs = self.encoder(tensor_obs).numpy()[0]
            return encoded_obs

        return raw_observation

    def set_autoencoder(self, encoder, scaler):
        """
